...
from c5_models import TakenCourse, UserInfo, CourseRegistrationInfo, UserStatistics
...
def test_integration_with_existing_components():
    ...
    try:
        # Test compatibility with user_manager.py
        print("\n--- user_manager.py互換性テスト ---")
        from user_manager import get_user_info, create_user_account, authenticate_user
        
        test_user_id = 77777
        test_password = "compat123"
        
        # Test account creation through old interface
        old_create_success = create_user_account(test_user_id, test_password)
        print(f"旧インターフェースでのユーザ作成: {'成功' if old_create_success else '失敗'}")
        
        # Test authentication through old interface
        old_auth_success = authenticate_user(test_user_id, test_password)
        print(f"旧インターフェースでの認証: {'成功' if old_auth_success else '失敗'}")
        
        # Test user info retrieval through old interface
        old_user_info = get_user_info(test_user_id)
        if old_user_info:
            print(f"旧インターフェースでのユーザ情報取得: 成功")
            print(f"  ユーザID: {old_user_info.user_id}")
            print(f"  科目数: {len(old_user_info.taken_courses)}")
        else:
            print("旧インターフェースでのユーザ情報取得: 失敗")
        
        print("✓ 既存コンポーネント統合: 正常動作")
        
    except Exception as e:
        print(f"✗ 既存コンポーネント統合 エラー: {e}")
